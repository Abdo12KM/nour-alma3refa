...
export async function verifyUserByPin(userId: number, pin: string): Promise<{ success: boolean; name?: string }> {
...
// Get user info by ID
export async function getUserById(userId: number): Promise<{ id: number; name: string } | null> {
  const result = await db.select({
    id: users.id,
    name: users.name,
  })
